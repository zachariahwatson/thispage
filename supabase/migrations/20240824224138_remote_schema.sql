alter table "public"."readings" add column "interval_section_length" bigint not null default '1'::bigint;
  ...
  if _all_complete then
    ...
    if _increment_type = 'pages' then
      ...
      if _old_goal_section = _book_sections then
        ...
        -- create new interval and return the id
        insert into public.intervals (reading_id, goal_section)
        values (_reading_id, _new_goal_section)
        returning id into _new_interval_id;
...
AS $function$begin
  ...
  values (new.id, new.interval_page_length, 1);
  return new;
end;$function$
;
