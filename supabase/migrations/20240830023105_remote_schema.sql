CREATE UNIQUE INDEX unique_member_poll_item ON public.poll_votes USING btree (member_id, poll_item_id);

alter table "public"."poll_votes" add constraint "unique_member_poll_item" UNIQUE using index "unique_member_poll_item";
...
CREATE TRIGGER decrement_total_votes_count_upon_poll_item_deletion AFTER DELETE ON public.poll_items FOR EACH ROW EXECUTE FUNCTION handle_poll_item_deletion();

CREATE TRIGGER update_votes_count_upon_poll_vote_update AFTER UPDATE ON public.poll_votes FOR EACH ROW EXECUTE FUNCTION handle_poll_vote_update();
