...
import { ThemeIntervalAvatarList } from "@/components/ui/themes"
...
export function ThemeIntervalAvatarGroup({ isComplete }: Props) {
			...
			{isVertical ? (
				<Sheet>
					...
					<SheetContent className="sm:max-w-xl max-w-xl w-full rounded-lg">
						<ThemeIntervalAvatarList isComplete={isComplete} />
					</SheetContent>
				</Sheet>
			) : (
				<Dialog>
			...
			<div className="flex flex-row -space-x-1">
				<Tooltip>
					<TooltipTrigger className="cursor-default">
						<Avatar className={`${isComplete ? "ring-ring" : "ring-page"} ring-4 size-8 md:size-10`}>
