	...
	return (
		<div id={`club-${clubMembership?.club.id}`} className="max-w-sm md:max-w-4xl w-full space-y-3 relative">
			...
			<Card
				...
				className="h-[calc(100svh-64px)] min-h-[624px] md:h-[624px] p-3 md:p-4 rounded-3xl relative shadow-shadow shadow-sm bg-card"
			>
				<Spreads />
			</Card>
			<ClubActionsButton />
...
export function ClubBookSkeleton() {
	return (
		<div className="max-w-sm md:max-w-4xl w-full space-y-3">
			...
			<Skeleton className="h-[calc(100svh-64px)] min-h-[624px] md:h-[624px] p-2 md:p-4 rounded-3xl" />
		</div>
	)
}
